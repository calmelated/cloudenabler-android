...
public class AccountActivity extends ActionBarActivity implements AdapterView.OnItemClickListener, PullToRefreshBase.OnRefreshListener2<ListView>, SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        curUserName = Utils.loadPrefs(context, "UserName");
        subCompID = Utils.loadPrefs(context, "SubCompID", "0");
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.new_account) {
            if (adapter.getCount() < PrjCfg.MAX_ACCOUNT_NUM) {
                startActivity(new Intent(context, NewAccountActivity.class));
            } else {
                MHandler.exec(mHandler, MHandler.SHOW_ERR_MSG, getString(R.string.err_msg_max_account_exceeded));
            }
        }
        return true;
    }
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        if(account.admin) {
            if(account.name.equalsIgnoreCase("Admin")) {  // the first user
                menu.add(0, R.id.edit, 0, getString(R.string.edit));
            } else { // other admin
                menu.add(0, R.id.edit, 0, getString(R.string.edit));
                if(curUserName.equals("Admin") || !subCompID.equals("0")) {
    ...
    private static class MsgHandler extends MHandler {
        public MsgHandler(AccountActivity activity) {
            super(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            final AccountActivity activity = (AccountActivity) super.mActivity.get();
            switch (msg.what) {
                ...
                case MHandler.ADD_LIST: {
                    activity.adapter.addList((Account) msg.obj);
                    break;
                }
                case MHandler.DEL_LIST: {
                    activity.adapter.remove((String) msg.obj);
                    break;
                }
